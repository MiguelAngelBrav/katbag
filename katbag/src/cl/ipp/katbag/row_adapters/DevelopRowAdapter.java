...
public class DevelopRowAdapter extends ArrayAdapter<String> {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (dev.size() > 0) {
			...
			if (subHumaName != null) {
				subHumaName.setText("(" + dev.get(7) + ") - id:" + items.get(position));
			}
		}
		...
		int levelColor = 0;
		if ((level >= 0) && (level <= 9)) levelColor = level;
		else if ((level >= 10) && (level <= 19)) levelColor = level - 10;
		else if ((level >= 20) && (level <= 29)) levelColor = level - 20;
		else if ((level >= 30) && (level <= 39)) levelColor = level - 30;
		else if ((level >= 40) && (level <= 49)) levelColor = level - 40;
		else if ((level >= 50) && (level <= 59)) levelColor = level - 50;
		else if ((level >= 60) && (level <= 69)) levelColor = level - 60;
		else if ((level >= 70) && (level <= 79)) levelColor = level - 70;
		else if ((level >= 80) && (level <= 89)) levelColor = level - 80;
		else if ((level >= 90) && (level <= 99)) levelColor = level - 90;
